...
// category   - internal node of some syntactic category (for Ex. either an if statement, or 
//a for loop that would have children nodes for the conditions being tested)

// token      - a source code token(so in the expression "2 + 3 * 4" each individual character is a token)
